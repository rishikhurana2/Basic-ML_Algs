...
class AlwaysPredictMostFrequent(BinaryClassifier):
    ...
    def train(self, X, Y):
        ...
        ### TODO: YOUR CODE HERE
        self.mostFrequentClass = 1 if np.sum(Y) >= 0 else -1
...
class FirstFeatureClassifier(BinaryClassifier):
    ...
    def train(self, X, Y):
        ...
        '''
        Y_X_lte_zero = np.array([])
        Y_X_gt_zero  = np.array([])
        for i in range(len(Y)):
            if X[i][0] <= 0:
                np.append(Y_X_lte_zero, Y[i])
            else:
                np.append(Y_X_gt_zero, Y[i])

        self.classForNeg = 1 if np.sum(Y_X_lte_zero) >= 0 else -1
        self.classForPos = 1 if np.sum(Y_X_gt_zero) > 0 else -1